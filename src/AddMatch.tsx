...
const allowed_players = ["Brendan", "Jared", "Bill", "Sean",]

export default function AddMatch() {
  const [players, setPlayers] = React.useState<string[]>(["Brendan", "Jared", "Bill", "Sean",]);
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const myplayers: Player[] = idx4.map(i => ({ name: players[i], general: generals[i] ?? General.UNRECOGNIZED, team: teams[i] }))
    const date = new Date()
    ...
    <form onSubmit={handleSubmit}>
      {idx4.map(i =>
        <FormControl sx={{ m: 1, minWidth: 140 }} id="test">
          ...
          >
            {allowed_players.map(p => <MenuItem value={p}>{p}</MenuItem>)}
          </Select>
        </FormControl>
      )
      }
      ...
      {idx4.map((i) =>
        <FormControl sx={{ m: 1, minWidth: 140 }}>
          ...
          >
            {[1, 2, 3, 4].map(p => <MenuItem value={p}>{p}</MenuItem>)}
          </Select>
        </FormControl>
      )
      }
      ...
      {idx4.map((i) =>
        <FormControl sx={{ m: 1, minWidth: 140 }}>
          ...
          >
            {Object.keys(General).filter(key => isNaN(Number(key))).map(g => <MenuItem value={g}>{g}</MenuItem>)}
          </Select>
        </FormControl>
      )
      }
      ...
      <FormControl>
        ...
        <RadioGroup
          ...
          name="winner"
          value={winner}
          onChange={(event) => setWinner((+event.target.value) as (1 | 2 | 3 | 4))}
        >
          {[1, 2, 3, 4].map((i) => <FormControlLabel value={i} control={<Radio />} label={`Team ${i}`} />)}
        </RadioGroup>
      </FormControl>
      <Divider />
      <FormControl sx={{ m: 1, minWidth: 140 }}>
        <TextField id="map" label="Map" variant="outlined" value={map} onChange={(event) => setMap(event.target.value)} />
      </FormControl>
      <Divider />
      <FormControl sx={{ m: 1, minWidth: 140 }}>
        <TextField
