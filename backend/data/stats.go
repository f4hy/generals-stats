...
func PairStats(matches *pb.Matches) *pb.PairsWinLosses {
        var pairstats pb.PairsWinLosses
         := make(map[string]map[pb.General]*pb.WinLoss)
	
        for _, m := range matches.Matches {
		for _, p := range getPairs(m){
			ps := pairstats.PairsWinLosses[p.team.String()]
			if(p.won){
				 ps.General1Wl.
			}
		}
